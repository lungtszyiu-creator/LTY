...
import { updateLtyLegalRequest, deleteLtyLegalRequest } from '../../_actions';

export const dynamic = 'force-dynamic';

export default async function LtyLegalRequestDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const ctx = await requireDeptView('lty-legal');
  const { id } = await params;
  const r = await prisma.ltyLegalRequest.findUnique({
    where: { id },
    include: {
  ...
  const updateAction = updateLtyLegalRequest.bind(null, r.id);
  const deleteAction = deleteLtyLegalRequest.bind(null, r.id);
      ...
      {canEdit ? (
        <section className="rounded-2xl border border-slate-200 bg-slate-50/50 p-4">
          ...
          <LegalRequestForm
            ...
            initial={initial}
            action={updateAction}
            cancelHref={`/dept/lty-legal/requests/${r.id}`}
          />
          {ctx.isSuperAdmin && (
            <div className="mt-4 border-t border-slate-200 pt-4">
              <ConfirmDeleteForm
                action={deleteAction}
                message={`永久删除需求「${r.title}」？\n该操作不可恢复。`}
              >
                <button
                  type="submit"
